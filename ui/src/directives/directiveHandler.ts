...
export abstract class BaseDirectiveHandler<T> implements IDirectiveHandler<T> {
    ...
    public applyDirective(node: Element, directive: IDirective<T>, state: INodeState) {
        ...
        if (this.unique && state.getDirectives(directive.name).length > 1) {
            exception.next(new Error(`more than 1 ${directive.name} directive on element ${node}`));
            return;
        }
        if (this.parametricity === Parametricity.Forbidden && directive.parameters.length > 0) {
            exception.next(new Error(`directive "${directive.name}" with expression "${directive.text}" on element ${node} can't have parameters`));
            return;
        } else if (this.parametricity === Parametricity.Required && directive.parameters.length === 0) {
            exception.next(new Error(`directive "${directive.name}" with expression "${directive.text}" on element ${node} must have a parameter`));
            return;
        }
...
export abstract class DirectiveHandler<T> extends BaseDirectiveHandler<T> {
    public applyInternal(node: Element, directive: IDirective<T>, state: INodeState) {
        ...
        if (this.dataFlow === DataFlow.In && !isObserver(obs)) {
            exception.next(new Error(`directive "${directive.name}" with expression "${directive.text}" on element ${node} must be supplied with an observer or a function`));
            return;
        }
